...
def test_artifact_lookup_is_404_across_tenant_boundary(
    ...
    artifact_id = uuid4()
    with psycopg.connect(langgraph_v2_migrated_database_url, autocommit=True) as connection:
        connection.execute(
    ...
    with TestClient(app) as client:
        own = client.get(f"/v2/artifacts/{artifact_id}", headers={"X-Application-Id": "tenant-a"})
        other = client.get(f"/v2/artifacts/{artifact_id}", headers={"X-Application-Id": "tenant-b"})
