...
- [x] 每个 round 配置一次 PydanticAI output retry，即一次 actor invocation 内最多 2 次 model requests；schema 与业务校验反馈保留在同一消息历史，被拒绝的 candidate 不计为 round，输入保持冻结。
- [x] 覆盖边界与多 1：最多 5 次 Coordinator decisions、4 个 dispatch rounds、32 个 Tasks、每个 Task 8 个 context result IDs。
- [x] 完成第 4 个 dispatch round 后只允许 `Finish`；非法 dispatch candidate 先收到一次 actionable output retry，仍非法则以 incomplete 结束且不创建 Send。
...
- [x] 每个 accepted decision 生成 immutable `CoordinationRound` 并单调增加 revision；被拒绝的 output candidate 不增加 revision、不生成 round，checkpoint round-trip 后保持一致。
- [x] Task objective 先做确定性的单行规范化，明确处理 CRLF、换行、控制字符与 NFC；保存 canonical value，后续 disclosure 复用同一值。

## Comments

- Test seam / scope review（gpt-5.6-sol medium）：范围限于 Ticket 10 的 rolling coordinator、accepted-round checkpoint、cross-round context 与 bounded output retry；不提前实现跨 Request admission/locking 或后续 ticket。测试 seam 为 pure acceptance/materialization、PydanticAI actor、真实 Agent graph/PostgreSQL checkpoint 与 request stream。
...
- 2026-09-07：按用户确认重构 retry ownership：删除外层 `range(2)`、可选 `repair()` 与 `CoordinatorOutputInvalid`；Coordinator 使用一次内建 output retry，`CoordinatorInput` 同时作为 prompt projection 与 validator deps，schema 错误保留 Pydantic 字段反馈，业务错误通过 `ModelRetry` 返回 actionable feedback。最终 acceptance 仍按 authoritative state 复核。
- 2026-09-07：actor invocation 定义为一次 adapter method + 一次 `Agent.run`，model request 定义为该 run 内的一次 provider call。Coordinator 每个 Decision 只有一次 actor invocation、至多两次 model requests；每次 request 独立应用 60 秒 timeout。
