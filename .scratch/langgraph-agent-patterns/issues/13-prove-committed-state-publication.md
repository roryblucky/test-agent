...
- [ ] structural reasons 只允许 `task_limit` 与 `coordination_limit`；原因与 disclosure 按稳定 Round、Task、unavailable-outcome 顺序排列。
- [ ] disclosure 只显示经 Markdown escaping 的 canonical failed-task objectives、Data Gap coverage labels、固定 insufficient-Evidence statement 与标准 structural-limit statement，不泄露 tool identity、arguments、provider details、exceptions、retries 或 counters。
