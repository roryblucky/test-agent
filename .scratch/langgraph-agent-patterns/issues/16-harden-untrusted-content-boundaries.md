...
- marker fixture 同时含 eligible/unknown Evidence 与 Calculation markers、fake citation、Artifact alias、Task/Run identity、provenance；断言 citation、Prepared aliases、Task/Run identity、Calculation provenance、provider source/query 与 allowed Tool set 仍由 code-owned typed bindings 决定。非法 marker 经一次 frozen Prepared native output retry 后仍 fail closed，绝不删 marker 后发布。
