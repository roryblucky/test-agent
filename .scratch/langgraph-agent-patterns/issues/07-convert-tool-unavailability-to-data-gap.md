...
**What to build:** 把已注册的预期 tool 不可用与 binding 自身超时转换为 typed、可审计的 Data Gap，使部分成功只能保守地收紧最终结论。
...
- [x] 无法按注册 contract 安全投影的 provider response 使用 `response_unusable`。
- [x] app-only metadata 保留完整 binding 字段，但不得包含 secrets、raw provider payload 或其他不可信内容。
...
- [x] 每个贡献最多 8 个 Data Gap；测试覆盖恰好上限与多 1。
