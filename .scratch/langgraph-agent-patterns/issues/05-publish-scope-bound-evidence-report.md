...
- [x] `ToolReturn` 将有界的 model-visible value 与 app-only typed Evidence metadata 分离，metadata 对模型不可见。
...
- [x] 基础 Synthesis 只接收有界 excerpt，输出带 marker 的 Markdown；代码校验 marker 语法与 Evidence eligibility，并由代码推导 citations。
...
- [x] 覆盖 cache 未超限路径；Evidence cache 溢出语义由 Ticket 08 完成。
...
- 2026-09-06: Implemented frozen registered ∩ Tenant ∩ Scope ∩ Specialist Tool
  ...
  acceptance, freshness gate, bounded ToolReturn, Synthesis publication gate, and
