...
- [x] 只支持 canonical spec 指定的 3 类 versioned calculation。
- [x] 模型只能提交受信任 refs；executor 解析 series，直接提交 raw series 的模型输出被拒绝。
- [x] calculation metadata 只从 accepted attempt 推广，并扩展 Ticket 08 的 attempt isolation 测试。
