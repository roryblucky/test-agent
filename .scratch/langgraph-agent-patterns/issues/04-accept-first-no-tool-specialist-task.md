...
- [x] 通用 `execute_specialist` 通过 typed registry 调用 actor，并产出最小 `TaskSucceeded`；usage、retry 与 diagnostics 不进入 `TaskOutcome`。
- [x] contribution 不可变；单一 barrier 校验 manifest、identity 与 attempt 后，在一个状态更新中 promote accepted contribution 并清空 staging。
- [x] accepted success 没有 Evidence 时复用 `insufficient_evidence` 的保守发布语义。
